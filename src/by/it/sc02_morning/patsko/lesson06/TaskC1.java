...
public class TaskC1 {
    public static void main(String[] args) {
        ...
        do {
            n = sc.nextInt();
            if(n<0&&n>62) System.out.println("Попробуйте еще раз");
        }
        while(n<0&&n>62);
        String zero="", binary;
        for(int i=1;i<=n;i++) {
        ...
        for(int i=0; i<=n;i++) {
            binary=zero+Long.toBinaryString(x);
            System.out.printf("%s = %d\n",binary,x);
            x<<=1;                           //x=1<<y работает не корректно(максимальный сдвиг на 30 бит(i=30))
            if(i!=n)zero=zero.substring(1);  //без условия - String index out of range: -1
        }
    }
}
